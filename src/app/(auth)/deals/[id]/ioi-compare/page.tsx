...
export default function IOIComparePage() {
  const params = useParams();
  const dealId = params.id as string;
  const [iois, setIois] = useState<IOI[]>([]);
  ...
  const rows: { label: string; key: string; format?: "currency" | "boolean" }[] = [
    ...
    { label: "Time to Close", key: "time_to_close" },
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.key} className="border-t border-border-gray">
                  ...
                  {iois.map((ioi) => {
                    const val = (ioi as unknown as Record<string, unknown>)[row.key];
                    let display: string;
                    if (val == null) display = "—";
                    ...
                    return (
                      <td key={ioi.id} className="px-4 py-3">{display}</td>
                    );
                  })}
                </tr>
              ))}
